...
class ProductImport extends React.Component {
		...
		api.products.list(filter).then(({ status, json }) => {
			...
			for (let i in json.data) {
				...
				if (json.data[i].images.length > 0) {
					api.products.images.delete(json.data[i].id, json.data[i].images[0].id);
				}
			}
		}).catch(error => {
			console.log(error);
	...
	uploadProducts() {
		...
		for (let i = 1; i < this.state.product_items.length; i++) {
			...
			if (this.state.product_items[i] !== undefined) {
				...
				productDraft.sub_category_name = this.state.product_items[i]['sub_category_name'];
				productDraft.name = this.state.product_items[i]['name'];
				productDraft.stock_quantity = this.state.product_items[i]['stock_quantity'];
				...
				updateProductArray.push({'category_name': productDraft.category_name, 'sub_category_name': productDraft.sub_category_name, 'draft': productDraft });
	...
	recreateCategories() {
		...
		updateProductArray.forEach((pArrayItem, i) => {
			api.productCategories.create({enabled: true, name: pArrayItem.category_name}).then(({status, json}) => {
				catArray.push(json);
	...
	removeCategoryDuplicates(catArray) {
		const that = this;
		let newCatArray = [];
		newCatArray = Object.values(catArray.reduce((acc, cur) => Object.assign(acc, {
			[cur.name]:cur
		}),{}));
		...
		newCatArray.forEach((item, i) => {
			api.productCategories.create({enabled: true, name: item.name}).then(({status, json}) => {
	...
	getRootCategoryIds() {
		...
		api.productCategories.list({enabled: true, fields: CATEGORIES_FIELDS}).then(({status, json}) => {
			updateProductArray.forEach((pArrayItem, i) =>  {
				...
				updateProductArray.forEach((elem, j) => {
					if (item.name === elem.category_name) {
						...
						if (elem.sub_category_name !== '') {
							api.productCategories.create({enabled: true, name: elem.sub_category_name, parent_id: item.id}).then(({status, json}) => {
	...
	updateProduct() {
		...
		updateProductArray.some((pArrayItem, i) => {
			...
			api.products.create(pArrayItem.draft).then(({ status, json }) => {
				imageFilesArray.push({'id': json.id, 'url': pArrayItem.draft.path.split(',')});
					...
					json.forEach(jFile => {
						if (imageFile === jFile.file) {
							...
							xhr.onload = function( e ) {
								...
								api.products.images.upload(aFile.id, form).then(({ status, json }) => {
									console.log(status);
								}).catch(error => {
									console.log(error);
								});
	...
	componentDidMount() {
		...
		api.settings.retrieveImportSettings().then(({ status, json }) => {
			...
			fetch(spreadsheetApiCredentials).then(response => response.json()).then(data => {
    ...
    render() {
			...
			if (j < 1) {
				return (
					<tr className="tr-header" key={keyCounter}>
						{Object.keys(p).filter(k => k !== 'id').map((k, i) => {
							return (
								<th className="td-header" key={keyCounter+i}>
									<div ref="status" className={k === 'state' ? "sheet-cell-state" : "sheet-cell-"+i} suppressContentEditableWarning="true" key={p[i]+j+i+p[j]} contentEditable="true" value={k} onInput={this.editColumn}>
										{p[k]}
									</div>
								</th>
							);
						...
						{Object.keys(p).filter(k => k !== 'id').map((k, i) => {
							return (
								<td className="td-body" key={keyCounter+i}>
									<div className={k === 'state' ? "sheet-cell-state" : "sheet-cell-"+i} suppressContentEditableWarning="true" key={p[i]+j+i+p[j]} contentEditable="true" value={k} onInput={this.editColumn}>
										{p[k]}
									</div>
								</td>
							);
		...
		return (
			<Fragment>
				<div className="spread-sheet-container" style={ this.state.productsImport }>
					<fieldset className="spread-sheet-table">
						<div className="heading">
							<h3>Google Spreadsheet Product Import</h3>
							<p> {this.state.product_items.length - 1} products are listed / {this.state.uploadedProducts} products are uploaded {this.state.errors > 0 ? '/ Errors accured ' + this.state.errors : ''}
								<span ref={this.loader} style={showLoader} className="loader loader-product-import">
						...
						<div className="schedule padd-lr">
							...
							<div className="tbl-content">
								...
								<h3 className="dashboardErrorResponse">{!this.state.dashboardsettings ? messages.missing_dashboardsettings : null}</h3>
								<p>{!this.state.dashboardsettings ? messages.setup_google_spreadsheet : null}</p>
							</div>
						</div>
					</fieldset>
					<FlatButton
