...
class CommerceForm extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if(nextProps.initialValues !== undefined && !this.state.isServiceOptionsCalled) {
			const isServiceOptions = nextProps.initialValues.status.indexOf(messages.commerce_formRestaurant) !== -1;
			const isTextField = nextProps.initialValues.serviceOptions.indexOf(selectFieldValuesFirst[1]) !== -1;
			this.setState({ isServiceOptions, isServiceOptionsCalled: true, isTextField });
		}
	}
	...
	setSelectField = (event) => {
		const isSelectField = event.target.value.indexOf(messages.commerce_formRestaurant) !== -1 || false;
			...
			>
				<Paper className="paper-box" zDepth={1}>
					<div className={style.innerBox}>
						...
						<div>
							{(this.state.isSelectField || this.state.isServiceOptions) && <Field name="serviceOptions"
							...
							>
								<MenuItem value={selectFieldValuesFirst[1]} primaryText={messages.service_delivery} />
								<MenuItem value={selectFieldValuesSecond[2]} primaryText={messages.service_togo} />
								<MenuItem value={selectFieldValuesThird[3]} primaryText={messages.service_delivery_togo} />
							</Field>}
						</div>
						<div>
							{(this.state.isTextField && (this.state.isSelectField || this.state.isServiceOptions)) && <Field
								component={TextField}
								fullWidth={true}
								name="deliveryRadius"
